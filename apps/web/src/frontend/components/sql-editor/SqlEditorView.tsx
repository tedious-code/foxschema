...
import { Loader2, Play, Eraser, AlignLeft, Columns2, Rows3, RefreshCw, BookmarkPlus, Database, Bookmark, Network, Shield } from 'lucide-react';
...
export const SqlEditorView: React.FC = () => {
  ...
  // Completion provider reads active SQL + checked schemas via this getter.
  useEffect(() => {
  ...
  const canRun = !runningTabId && runCount > 0 && liveSelectedIds.length > 0;
  ...
  return (
    <div className="flex-1 flex min-h-0 overflow-hidden" data-testid="sql-editor-view">
      <aside className="w-72 shrink-0 border-r border-slate-800 bg-slate-925/40 overflow-hidden flex flex-col min-h-0">
        <SqlSidebarSection
          id="destinations"
