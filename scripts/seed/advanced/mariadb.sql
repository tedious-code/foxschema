-- FoxSchema ADVANCED seed — MariaDB 10.6+
-- Database pair: demo_c (source) vs demo_d (target).
-- Case matrix: see scripts/seed/advanced/postgres.sql (B1–B14).
-- MariaDB deviations: B6 skipped (no materialized views); B5 INCLUDED —
-- MariaDB has native sequences (unlike MySQL) and this exercises FoxSchema's
-- MariaDB sequence introspection + NOCYCLE/NOCACHE generation.
-- Load via: bash scripts/seed/seed-advanced.sh mariadb
...
-- [B5] Sequence with every attribute set (MariaDB wants NOCACHE/NOCYCLE as single tokens)
CREATE SEQUENCE seq_batch START WITH 500 INCREMENT BY 5 MINVALUE 1 MAXVALUE 1000000 CACHE 20 CYCLE;
...
-- [B5] Same sequence name, different attributes (plain defaults, NOCACHE NOCYCLE)
